...
func TestFlipPluginRegister(t *testing.T) {
	...
	_, exists := env.GetDocs("flip", docs.TypeProcessor)
	require.True(t, exists)
}

func TestFlipPluginExecute(t *testing.T) {
	if len(pluginManifest) == 0 || len(pluginWasm) == 0 {
		t.Skip("skipping plugin tests: requires plugin.wasm and plugin.yaml in testdata/")
	}
