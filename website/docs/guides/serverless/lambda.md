...
**Note: if you are using [partial batch responses](https://docs.aws.amazon.com/lambda/latest/dg/services-sqs-errorhandling.html#services-sqs-batchfailurereporting) then 
...
First, configure some stubbed environment variables:
```sh
...
To run commands against LocalStack, just [configure the endpoint URL](https://docs.localstack.cloud/user-guide/integrations/aws-cli/#configuring-an-endpoint-url) `--endpoint-url=<localstack-url>` flag in CLI calls to point to LocalStack.
