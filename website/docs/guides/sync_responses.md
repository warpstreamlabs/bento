...
In a regular Bento pipeline messages will flow in one direction and acknowledgements flow in the other:
...
It's possible to route the result of any Bento processing pipeline directly back to an input with a [`sync_response`][sync-res] output:
...
However, it is important to keep in mind that due to Bento' strict delivery guarantees the response message will not actually be returned until the message has reached its output destination and an acknowledgement can be made.
