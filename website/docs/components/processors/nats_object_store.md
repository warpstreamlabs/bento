...
:::
Perform operations on a NATS object store bucket.
...
The operation to perform on the Object Store bucket.
...
| Option | Summary |
|---|---|
| `get` | Returns the latest value for the `object_name`. |
| `put` | Places a new value for the `object_name` in the object store. |
