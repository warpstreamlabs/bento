...
func cacheCollectorProc(confStr string) (*cacheCollector, error) {
	pConf, err := cacheCollectorSpec().ParseYAML(confStr, nil)
	if err != nil {
		return nil, err
	}
	return newCacheCollector(pConf, service.MockResources(service.MockResourcesOptAddCache("test_cache")))

}

func TestCacheCollectorBasicAccumulation(t *testing.T) {
	t.Run("basic_with_delete", func(t *testing.T) {
		spec := `
resource: test_cache
key: test_key
init: root = [{"id":"0"}]
...
flush_deletes: true
`
		processor, err := cacheCollectorProc(spec)
	...
	t.Run("basic_without_delete", func(t *testing.T) {
		spec := `
resource: test_cache
key: test_key
init: root = [{"id":"0"}]
