...
// downloading the python wasm runtime to the ./artifacts directory.
//
// Note: the usage of the embed directive means that bento will not compile if the ./artifacts directory is not present.
// We do not embed the entrypoint.py and python-3.12.0.wasm files directly since this would prevent compilation on a local
// dev environment if the .wasm does not exist (i.e has not yet been downloaded).
