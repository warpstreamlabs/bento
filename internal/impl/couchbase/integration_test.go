...
func requireCouchbase(tb testing.TB) string {
	integrationOnce.Do(func() {
		...
		integrationCleanup = func() error {
			return pool.Close(context.WithoutCancel(context.Background()))
		}
	})
...
func setupCouchbase(tb testing.TB) (dockertest.ClosablePool, dockertest.Resource, error) {
	tb.Log("setup couchbase cluster")

	pool, err := dockertest.NewPool(tb.Context(), "", dockertest.WithMaxWait(time.Minute))
	if err != nil {
		return nil, nil, err
	}
	...
	resource, err := pool.Run(tb.Context(), "couchbase",
		dockertest.WithTag("latest"),
		dockertest.WithCmd([]string{"/opt/couchbase/configure-server.sh"}),
		...
		dockertest.WithPortBindings(dockernetwork.PortMap{
			dockernetwork.MustParsePort("8091/tcp"): {
				{HostIP: netip.MustParseAddr("0.0.0.0"), HostPort: "8091"},
			},
			dockernetwork.MustParsePort("11210/tcp"): {
				{HostIP: netip.MustParseAddr("0.0.0.0"), HostPort: "11210"},
			},
		}),
	...
	for {
		...
		res, err := resource.Exec(tb.Context(), []string{"/usr/bin/cat", "/is-ready"})
