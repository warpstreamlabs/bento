...
func CommonFields() []*service.ConfigField {
	return []*service.ConfigField{
		service.NewURLField("url").Description("Couchbase connection string.").Example("couchbase://localhost:11210"),
		service.NewStringField("username").Description("Username to connect to the cluster.").Optional(),
		service.NewStringField("password").Description("Password to connect to the cluster.").Secret().Optional(),
		service.NewStringField("bucket").Description("Couchbase bucket."),
		service.NewStringField("collection").Description("Bucket collection.").Default("_default").Advanced().Optional(),
		service.NewStringAnnotatedEnumField("transcoder", map[string]string{
		...
		service.NewDurationField("timeout").Description("Operation timeout.").Advanced().Default("15s"),
	}
}
