...
`).Example("Decode kafka_key metadata", "An example of decoding the kafka_key using a [branch](/docs/components/processors/branch) processor", kafkaDecodeKeyExampleConfig).
		...
		Field(service.NewBoolField("sanitize_namespace_names").Description("When enabled, sanitizes namespace names to only contain letters, numbers, and underscores.").Default(false).Version("1.17.0").Advanced())
