...
func Spec() docs.FieldSpecs {
	return docs.FieldSpecs{
		...
		docs.FieldBool(fieldLogAllErrors, "Experimental: When set, the logger will promote any log containing an `error` type to the `ERROR` level.").HasDefault(false).Advanced().AtVersion("1.3.0"),
	}
}
