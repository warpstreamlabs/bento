...
class EditorManager {
      ...
      if (typeof window.getBloblangSyntax === "function") {
        try {
          ...
          if (syntaxData && !syntaxData.error) {
            ...
            console.warn("WASM syntax function returned error:", syntaxData?.error);
          }
        } catch (error) {
  ...
  configureFallbackEditors() {
    ...
    this.getElement(DOM_IDS.fallbackInput)?.addEventListener("input", () => this.handleInputChange());
    this.getElement(DOM_IDS.fallbackMapping)?.addEventListener("input", () => this.handleMappingChange());
  }
   ...
   */
  setMapping(content) {
    this.setEditorValue(this.aceMappingEditor, DOM_IDS.fallbackMapping, content);
  }
